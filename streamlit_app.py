...
# 장입모드 선택
# ------------------------------
st.sidebar.header("장입모드 선택")
mode = st.sidebar.radio("장입 방식 선택", ["장입속도 기반 (자동)", "현재 누적 Charge 직접입력"])

if mode == "장입속도 기반 (자동)":
    charge_rate = st.sidebar.number_input("장입속도 (charge/h)", value=5.5)
    elapsed_charges = charge_rate * (elapsed_minutes / 60)
else:
    elapsed_charges = st.sidebar.number_input("현재까지 누적 장입 Charge 수", value=30.0)
...
gen_series = [(ore_per_charge * (charge_rate if mode=="장입속도 기반 (자동)" else elapsed_charges/(elapsed_minutes/60)) * (t/60)) * (tfe_percent/100) * reduction_eff_adj * (1+1/slag_ratio) for t in time_labels]
...
st.download_button("📥 CSV 다운로드", data=csv, file_name="조업리포트_5_9_pro_plus.csv", mime='text/csv')
